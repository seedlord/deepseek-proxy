...
- **实时 TUI** — 实时状态栏，显示端口、请求数、模型、分离的 MAIN/SUB 缓存命中率、开关状态、运行时间和快捷键。可滚动的日志区域，带颜色编码的请求/响应行。
...
- **独立缓存追踪** — MAIN 和 SUB 缓存命中率独立追踪（它们拥有完全独立的上下文缓存）。
- **热重载** — 无需重启进程即可重载所有 `lib/*` 模块。日志缓冲区和终端状态得以保留。
- **翻页模式** — 冻结日志并通过类 vim 按键（`j`/`k`、`g`/`G`、`PgUp`/`PgDn`）滚动历史记录。状态栏保持实时更新。
- **会话检测** — 自动检测 API 密钥变更，并为新会话重置 MAIN 缓存统计。
...
proxy.js ── 编排器（HTTP 服务器、转发、键盘输入）
  ...
  ├── lib/tui.js       — 终端 UI：状态栏、翻页/回滚、缓存统计、节流重绘
  ├── lib/inspector.js — 解析 Claude API JSON 负载，提取模型/思考/工具信息
  └── lib/metrics.js   — 从流式响应缓冲区提取 token 用量，写入 CSV
```

**请求流程：** 客户端 → HTTP 服务器 → 带大小限制的请求体读取 → JSON 解析 → 负载检查 → 会话检测（通过 auth 头指纹）→ 子代理思考覆盖（如适用）→ 通过 HTTPS keep-alive 转发至 DeepSeek → 流式响应 → 从尾部缓冲区提取指标 → TUI 日志 + 分离的 MAIN/SUB 缓存统计 + CSV 追加。
...
- Node.js ≥ 18
- 无 npm 依赖 — 仅使用 `http`、`https`、`fs`、`readline` 内置模块
