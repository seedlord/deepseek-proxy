...
// 7.0: Session-based cache tracking — each API-key fingerprint gets
 ...
 *  Pass knownLen to skip the regex-based visibleLen scan (7.1: cached for repaint). */
...
function repaint() {
  ...
  try {
    ...
    // 7.1: use cached visLen — no regex per line. Falls back to raw string
...
// 7.1: store { raw, visLen } to avoid regex on every repaint
function logLine(s) {
...
// ── Header (6a: compact 4-line, 6b: avg hit rate, 6f: activity, 7.0: per-session) ──
...
// ── Stats tracking (7.0: per-session via fingerprint) ──────────────
...
function forceRepaint() {
  try {
    ...
    readline.cursorTo(process.stdout, 0, HEADER_ROWS);
    readline.clearScreenDown(process.stdout);
    // 7.1: use cached visLen, with string fallback for hot-reload compat
    for (const line of visible) {
      if (typeof line === 'string') writeLine(line);
      else writeLine(line.raw, line.visLen);
    }
